...
class ChatCLI:
    ...
    def __init__(self, model_list, db_file, table_name = "memory", session_id = None):
        ...
        self.conversation_starter = f"This is the beginning of a conversation between {self.num_models} different chatbot(s) and a single human. \
        ...
        # self.conversation_manager.conversation_history.append({"role": "user", "content": conversation_starter})
    ...
    def chat_step(self, user_input, model_idx):
        ...
        """
        overall_user_input = self.conversation_starter
        if user_input:
